...
#define BREAK_POINT 150 // exit after so many cycles -- useful for debugging
#define I_TYPE 2
...
void FSM()
{
    ...
    switch (state) {
        ...
        case DECODE:
            ...
            control->ALUSrcB = 3;
            control->ALUOp = 0;
            if (IR_meta->type == R_TYPE || opcode == SLT ) state = EXEC;
            else if (opcode == SW || opcode == ADDI || opcode == LW) state = MEM_ADDR_COMP;
        ...
        case MEM_ADDR_COMP:
            ...
            control->ALUSrcB = 2;
            control->ALUOp = 0;
            if(opcode == ADDI) state = I_TYPE_COMPL;
...
void execute()
{
    ...
    switch (control->ALUOp) {
        ...
        case 1:
            if(alu_opA == alu_opB){
              if(control->PCWriteCond && control->PCSource){
        ...
        case 2:
            ...
            if(IR_meta->opcode == SLT){
              destenation = alu_opA < alu_opB ? 1 : 0;
              next_pipe_regs->ALUOut = destenation;
              printf("%d - %d \n", alu_opA, alu_opB);
            }
            break;
        default:
    ...
    switch (control->PCSource) {
        ...
        next_pipe_regs->pc = (curr_pipe_regs->pc & (15<<28)); // 15<<28 is            11110000000000000000000000000000
                                                              //curr_pipe_regs->pc is 00000000000000000000000000111100
                                                              // curr_pipe_regs->pc & (15<<28) will

        next_pipe_regs->pc = (next_pipe_regs->pc | (IR_meta->jmp_offset<<2));
        break;
        default:
            assert(false);
    }
...
void memory_access() {
  ...
  if(control->MemRead && control->IorD){
    ...
    read_reg = memory_read(curr_pipe_regs->ALUOut);

  }
  if(control->MemWrite && control->IorD){
      int write_reg_id = arch_state.IR_meta.reg_16_20;
      memory_write(curr_pipe_regs->ALUOut, arch_state.registers[write_reg_id]);
  }
}
...
void set_up_IR_meta(int IR, struct instr_meta *IR_meta)
{
    ...
    switch (IR_meta->opcode) {
        ...
        case EOP:
            ...
            break;
        case SW:
            printf("Executing SW(%d), sw $%u, %u($%u)  (function: %u)\n",
            IR_meta->opcode,IR_meta->reg_16_20, IR_meta->immediate,IR_meta->reg_21_25,IR_meta->opcode);
            break;
        case ADDI:
              printf("Executing ADDI(%d), $%u = $%u + %u (function: %u) \n",
              IR_meta->opcode, IR_meta->reg_16_20, IR_meta->reg_21_25, IR_meta->immediate, IR_meta->opcode);
              break;
        case BEQ:
              printf("Executing BEQ(%d), beq $%u, $%u, %u  (function: %u) \n",
             IR_meta->opcode, IR_meta->reg_21_25, IR_meta->reg_16_20, IR_meta->immediate, IR_meta->opcode);
             break;
        case LW:
              printf("Executing LW(%d), lw $%u, %u($%u)   (function: %u) \n",
             IR_meta->opcode, IR_meta->reg_16_20, IR_meta->immediate,IR_meta->reg_21_25, IR_meta->opcode  );
             break;
        case J:
              printf("Executing J(%d), j %u \n",
              IR_meta->opcode,IR_meta->jmp_offset);
              break;
        case SLT:
              printf("Executing SLT(%d), slt $%u, $%u, $%u (function: %u)  \n",
             IR_meta->opcode, IR_meta->reg_11_15, IR_meta->reg_21_25,  IR_meta->reg_16_20, IR_meta->function);
             break;
        default: assert(false);
    }
